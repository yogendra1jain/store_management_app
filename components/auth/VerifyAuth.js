...
function mapStateToProps(state) {
    ...
    console.log(state,_get(state, '.commonReducer.userData.token', ''),"statestatestate")
    let { auth, nativeAuth } = state;
    let token = '';
    if (!_isEmpty(_get(state, 'commonReducer.userData.token', ''))) {
        token = _get(state, 'commonReducer.userData.token', '');
    }
    return {
        auth,
        nativeAuth,
...
export default connect(mapStateToProps)(withNavigation(VerifyAuth));
